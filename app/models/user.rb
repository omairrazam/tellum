class User < ActiveRecord::Base
  ...
  def user_created_and_following_drops
    (Rating.where("user_id = ?", self.id) + following_drops_with_is_anonymous_false).collect{|drop|
      {box_id: drop.try(:tag).try(:id), is_drop_story: true, box_name: drop.try(:tag).try(:tag_line), sort_created_at: (drop.try(:tag).try(:created_at) - (9.minutes + 42.seconds)),
