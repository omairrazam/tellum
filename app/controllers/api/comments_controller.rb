  ...
  def create
    ...
    if @comment.save
      ...
      unless rating_creator_user_id.try(:user).id == @user
        APNS.send_notification(rating_creator_user_id.try(:user).try(:device_token), alert: "#{rating_creator_user_id.try(:user).try(:full_name)} commented on #{rating_creator_user_id.try(:tag).try(:tag_line)}",badge: badge_count, sound: "default" )
